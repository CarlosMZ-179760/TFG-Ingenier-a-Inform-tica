...
reps=int(repsStr)
driftsList=[20,30,40]#np.arange(20, 30,10)
...
for detectorName in all_detectors:
    ...
    for stream_head in itertools.product(seeds,low_error_rate, magnitude_of_change, noise_stable_concept):
        ...
        for stream_tail in itertools.product(stable_concept_duration, duration_of_change,driftsList):
            ...
            generator=streamGen.GenericChangeGenerator(apparentDelay,instance_random_seed=stream[0], low_error_level=stream[1], incr_error_level=stream[2], noise_stable_concept=stream[3], duration_stable_concept=stream[4], duration_change=stream[5])
